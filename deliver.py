...
def _send_email(matches: list[Job], problems: list[str] | None = None) -> None:
    ...
    try:
        ...
        # Resend puts the real reason in the response body (JSON). The bare
        # status code alone ("403 Forbidden") isn't enough to diagnose, so
        # surface the body and the from-address we tried to send with.
        try:
            detail = e.read().decode("utf-8", errors="replace")
        except Exception:  # noqa: BLE001
            detail = "(could not read response body)"
        print(f"[deliver] email failed: HTTP {e.code} from={config.EMAIL_FROM!r} "
